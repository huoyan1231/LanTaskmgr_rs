...
```json
{
  ...
  "bind": "0.0.0.0",       // 绑定地址；0.0.0.0 = 监听所有网卡（手机可访问）
...
`bind` 决定 HTTP 服务监听哪个网络接口：

- 默认 `0.0.0.0`：监听**所有网卡**——这是手机能跨局域网访问的前提，也是唯一推荐的家庭用法。
- 填**具体局域网 IP**（如 `192.168.1.50`）：服务只在该地址监听。此时换到别的网卡（如你插上的公共
  Wi-Fi、酒店/咖啡厅网络）webui 根本不会在那个接口上出现，等于从源头**掐断了把管理页面暴露到公网/陌生网络的可能**。

> 桌面设置页新增「绑定到 IP」输入框：空着或格式非法会自动回退到 `0.0.0.0`，不会因填错而把服务绑到不可预期的地方。
> 注意：即便绑定到具体 IP，流量依旧**明文、无加密**，请勿在公共网络下使用，更不要做路由器端口映射。
